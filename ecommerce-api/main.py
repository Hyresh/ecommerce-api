...
async def list_products(
    ...
    in_stock: bool = Query(
        default=None, description="Filtrer par disponibilité en stock"
    ),
):
    query = {}
    if category:
...
async def search_products(
    q: str = Query(
        ..., description="Mot-clé à rechercher dans le nom et la description"
    )
):
...
async def low_stock_products(
    threshold: int = Query(
        default=10, description="Stock max pour être considéré faible"
    )
):
    cursor = products_collection.find(
        {"stock_count": {"$lte": threshold}, "in_stock": True}
    )
    results = [product_helper(p) async for p in cursor]
